...
import process from "process"; // ✅ Fix 'process is not defined' error
dotenv.config();
// Debugging logs
...
console.log("🚀 TWILIO_VERIFY_SID:", process.env.TWILIO_VERIFY_SID || "❌ NOT FOUND");

if (!process.env.TWILIO_ACCOUNT_SID || !process.env.TWILIO_AUTH_TOKEN || !process.env.TWILIO_VERIFY_SID) {
  console.error("❌ ERROR: Missing Twilio environment variables. Check your .env file!");
  process.exit(1);
}
...
// ✅ Fix Twilio Variables
const accountSid = process.env.TWILIO_ACCOUNT_SID?.trim() || "";
const authToken = process.env.TWILIO_AUTH_TOKEN?.trim() || "";
const verifySid = process.env.TWILIO_VERIFY_SID?.trim() || "";
...
app.post("/send-otp", async (req, res) => {
  ...
  try {
    const otpResponse = await client.verify.v2.services(verifySid)
      .verifications.create({ to: phone_number, channel: "sms" });

    console.log("📩 OTP Sent:", otpResponse.status);
    res.status(200).json({ message: "OTP sent successfully.", status: otpResponse.status });

  } catch (error) {
    console.error("❌ Error sending OTP:", error.message);
    res.status(500).json({ error: error.message });  // ✅ Send error message back
  }
});
...
app.post("/verify-otp", async (req, res) => {
  ...
  try {
    const verificationResponse = await client.verify.v2.services(verifySid)
      .verificationChecks.create({ to: phone_number, code: otp_code });

    if (verificationResponse.status === "approved") {
      console.log("✅ OTP Verified:", verificationResponse.status);
      res.json({ message: "OTP verified successfully." });
    } else {
      console.log("❌ Invalid OTP:", verificationResponse.status);
